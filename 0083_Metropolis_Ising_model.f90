  !!It takes about 1 min to run, since we sweep the spin network for 500000 times!!
MODULE CSEED
  INTEGER :: ISEED
END MODULE CSEED
PROGRAM Ising
...
!!!!!!!!!!!!!!! To sweep 5000 times of Monte Carlo
     DO I = 1, MAX
        INITIAL = E(SPIN, MAX)
        ELEMENT = RANF()*MAX+1
!!!!!!!!!!!!!!     take out one of the particles
        SPIN(ELEMENT) = -1*SPIN(ELEMENT)
        FINAL = E(SPIN, MAX)
        IF ((FINAL > INITIAL) .AND. (EXP((-(FINAL-INITIAL))/KT) < RANF())) THEN
!!!!!!!!!!!      Metropolis ( important sampling)
           SPIN(ELEMENT) = -1*SPIN(ELEMENT)
!!!!!!!!!!!!    if not accept, then start over
        END IF
     END DO
  ...
  ! !!!!!!!!!!!!      MT/MAX/5000 = the avg rate of the magnetization
END PROGRAM Ising
